...
const containerFadeIn: Variants = {
    ...
    <>
      ...
      <main className="flex flex-col md:flex-row items-center justify-between px-6 py-10 xl:py-40 max-w-7xl mx-auto overflow-hidden">
        <LazyMotion features={domAnimation}>
          ...
          >
            <m.span
              className="text-xs font-medium uppercase bg-green-800 text-white px-4 py-2 rounded-xs rounded-tl-xl rounded-br-xl sm:rounded-tl-2xl sm:rounded-br-2xl inline-block"
              variants={textFadeIn}
              custom={0}
            >
            ...
            <m.div className="mt-6 flex gap-4" variants={textFadeIn} custom={3}>
              <button
                className="bg-green-800 text-white px-5 py-2 hover:bg-[var(--color-primary-dark)] rounded-tl-xl rounded-br-xl transition cursor-pointer"
                onClick={goToPlants}
              >
                Explore Plants
              </button>
