        ...
        <div className="flex flex-col gap-2 sm:grid sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 sm:gap-3 px-2 mt-4">
          {cartItems.map((plant: CartItem) => {
            ...
            return (
              <div
                key={plant.variantId}
                className="flex rounded-md shadow-sm border border-[var(--color-primary-light)] hover:shadow-lg transition-all duration-300 relative"
              >
                <div className="relative w-[50%]  aspect-[5/3] overflow-hidden rounded-l-md">
                  {isLoading && (
                ...
                <div className="p-3 w-[50%] flex flex-col gap-1">
                  ...
                  <div className="mt-2 flex justify-between items-center w-full">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-1 sm:gap-2 text-sm mt-1">
                        {Array.from({ length: 5 }, (_, i) => {
                          const filled = i + 1 <= Math.floor(variant.ratings ?? 0);
                          const half = i + 0.5 === (variant.ratings ?? 0);
                          return (
                    ...
                    <div
                      onClick={e => {
                        e.stopPropagation();
                        goToPlantDetails(encryptId(plant.plantId), encryptId(plant.variantId));
                      }}
